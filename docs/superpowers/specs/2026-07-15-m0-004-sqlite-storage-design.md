...
M0-004 establishes ItemAll's local-first persistence foundation. It must support future identity, asset, CredentialRef, Operation Bus, AuditEvent, Task Engine, Evidence and Runbook modules without creating any of those business schemas yet.
...
- Use SQLite in WAL mode through a pure-Go driver.
...
- The repository already has deterministic Ubuntu and Windows CI gates.
- Avoiding GCC/MinGW and CGO environment requirements reduces local setup and release complexity.
- The driver integrates with `database/sql` and supports DSN-level connection pragmas.

The implementation plan must pin the selected `modernc.org/sqlite` version and verify the resolved `modernc.org/libc` dependency in `go.mod` and `go.sum`.

### 4.2 Migration mechanism

Use embedded SQL plus a project-local minimal runner.

Do not introduce goose, golang-migrate or another general migration framework. The runner is intentionally limited to:
...
The default path must never depend on the current working directory or executable directory.

Tests and explicit development wiring may inject a database path. Ordinary environment variables must not silently override the production location.

### 4.4 Transaction model

Use `Store.WithTx` with callback-managed transactions.

Callers do not manually commit or roll back and do not receive `*sql.Tx`. Application-level write transactions are serialized inside Store to reduce SQLite writer contention.
...
`internal/storage` defines portable contracts. `internal/storage/sqlite` owns every SQLite-specific detail.

Future domain repositories depend on `storage.Querier`, not on SQLite types.

## 6. Public internal contracts
...
- callback returning an error rolls back and preserves the callback error;
...
Use driver-supported DSN options so every created connection receives required connection-scoped settings. The implementation must explicitly verify effective values after opening.

Additional decisions:

- use immediate write transaction locking where supported;
- disable permissive double-quoted string compatibility if the driver option is available and stable;
...
## 9. Migration format and validation
...
- SHA-256 over the embedded raw SQL bytes;
...
Earlier successfully committed versions remain recorded. The application still fails closed until the broken migration is corrected by changing unapplied code or adding a later migration as appropriate. Applied history is never rewritten.

The first migration creates only migration/storage metadata needed to prove the foundation. It must not pre-create future domain tables.
...
- Cause;
- optional local Path;
- diagnostic metadata safe for local logs.
...
- upgrade from previous migration version;
- duplicate version rejection;
- version gap rejection;
- malformed and empty migration rejection;
- checksum mismatch rejection;
- schema-too-new rejection;
- migration failure rollback;
- real foreign-key enforcement;
- WAL mode verification;
- effective busy timeout and synchronous mode verification;
...
- callback error rollback;
- panic rollback and re-panic;
...
- race coverage for pure-Go storage code where compatible with the driver and current CI policy;
- existing `quality`, `generated-and-security` and `windows-build` jobs remain required;
- dependency policy and full-history secret scanning remain green;
...
- File permissions should use the most restrictive practical mode on each platform without claiming encryption-at-rest.
...
Future migrations add workspace, actor, device and version fields as defined by their owning tasks. M0-004 does not speculate on those schemas beyond preserving a clean migration and repository extension point.

## 15. Acceptance criteria

M0-004 is complete only when:

- a fresh install creates the local database and applies all migrations;
